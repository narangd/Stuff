...
;~ 		Local $sWinName = '장치 관리자'
;~ 		WinActivate($sWinName)
;~ 		Local $sText = ControlGetText($sWinName, '', 350)

;~ 		MsgBox(0, '', $sText)

;~ Local $hDriver = WinWaitActive('[CLASS:#32770]')
;~ Local $hWindow = WinWaitActive('[CLASS:MMCMainFrame]', '로컬 컴퓨터의 장치 관리자')

;~ 		Send('{APPSKEY}{R}')

;~ Local $command = 'devmgmt.msc'
;~ Local $hPID =  Run(@ComSpec & ' /c ' & $command, '', @SW_HIDE)
;~ WinWaitActive('[CLASS:MMCMainFrame]', '로컬 컴퓨터의 장치 관리자')
;~ 		Send('{TAB}{HOME}{APPSKEY}{A}')
;~ WinWaitActive('[CLASS:MMCMainFrame]', '로컬 컴퓨터의 장치 관리자')
;~ WinClose('[CLASS:MMCMainFrame]', '로컬 컴퓨터의 장치 관리자')

;MsgBox(1, '확인', '드라이버를 삭제하고 다시 검색하시겠습니까?')
